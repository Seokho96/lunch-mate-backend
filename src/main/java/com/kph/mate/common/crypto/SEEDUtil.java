...
//    System.out.println("암호화된 바이트 종료");
  ...
  private static List<byte[]> getByteList(String nomal, boolean isDecode) throws Exception{
    ...
    if(isDecode){
      ...
      gTempByte = base64Dec.decode(nomal);

//      BASE64Decoder base64Dec = new BASE64Decoder();
//      gTempByte = base64Dec.decodeBuffer(nomal);
      
      /* 암호화시 추가된 */
      int byteLen = gTempByte.length;
      byte checkValue = gTempByte[byteLen-1];

      if(checkValue==0x0f){
        tempByte = new byte[byteLen-1];
        for(int i=0; i<tempByte.length; i++){
          tempByte[i] = gTempByte[i];   
        }
      }else{
        throw new Exception();
      }
  ...
  private static String getByteListStr(List byteList, boolean isEncode) throws Exception{
    ...
    if(isEncode){
      byte[] resultByte = new byte[listByte.length - blankCnt + 1]; // 1바이트는 암호화 판단을 위해 추가
      
      for(int i=0; i<resultByte.length-1; i++){
          resultByte[i] = listByte[i];
      }   
      resultByte[resultByte.length-1] = 0x0f; // 판단위해 0x0f값 추가

      Base64.Encoder base64Enc = Base64.getEncoder();
      retStr = base64Enc.encodeToString(resultByte);
